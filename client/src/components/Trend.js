...
const Trend = ({ products }) => {
    ...
    return (
        <div className="flex flex-col justify-center items-center mt-10 md:mt-20 mb-10 md:mb-20">
            <h1 data-aos='fade up' className="text-3xl md:text-5xl"><span className="font-bold text-red-900">Popular</span> on <span className="font-['Lobster'] text-3xl md:text-5xl" >Music</span></h1>
            <Carousel
                ...
                {products.map((product) => (
                    <Link key={product._id} to={`/product/${product.name}`}>
                        <div data-aos='fade up' className="card rounded-md mx-1 md:mx-2 pb-2 transition-all cursor-pointer bg-white shadow hover:shadow-lg">
                            ...
                            <div className='w-4/5 mx-auto'>
                                <h1 className="title flex items-center h-10 text-basetransition-all">{product.name}</h1>
                                <p className="text-lg md:text-2xl mt-2 font-bold md:mb-10">{product.price}€</p>
                            </div>
                        </div>
                    </Link>
                ))}
